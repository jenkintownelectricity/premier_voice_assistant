...
export default function AssistantsPage() {
  ...
  // Skills - both HIVE Preloaded and Fast Brain
  const [availableSkills, setAvailableSkills] = useState<{id: string, name: string, description?: string, category?: string}[]>([]);
  const [hivePreloadedSkills, setHivePreloadedSkills] = useState<{id: string, name: string, description?: string}[]>([]);
  const [fastBrainSkills, setFastBrainSkills] = useState<{id: string, name: string, description?: string}[]>([]);
  const [fastBrainConfigured, setFastBrainConfigured] = useState(false);
  ...
  // TTS Providers - metadata only, voices fetched dynamically
  const TTS_PROVIDERS: Record<string, {
    ...
    cartesia: {
      name: "Cartesia (Recommended)",
      description: "Ultra-low latency, 42 languages",
      latency: "~30ms TTFB",
      default_voice: "f786b574-daa5-4673-aa0c-cbe3e8534c02",
    },
    elevenlabs: {
      name: "ElevenLabs",
      description: "High-quality, expressive voices",
      latency: "~150ms TTFB",
    ...
    openai: {
      name: "OpenAI TTS",
      description: "GPT-powered voice synthesis",
      latency: "~200ms TTFB",
      default_voice: "alloy",
    },
    playht: {
    ...
    rime: {
      name: "Rime AI (Free Tier)",
      description: "Fast, affordable TTS",
      latency: "~60ms TTFB",
      default_voice: "mist",
    },
  };
  ...
  const [voiceId, setVoiceId] = useState('f786b574-daa5-4673-aa0c-cbe3e8534c02');
  ...
  const loadSkills = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // Set all skills for backwards compatibility
        setAvailableSkills(data.skills || []);
        // Set categorized skills
        setHivePreloadedSkills(data.hive_preloaded || []);
        setFastBrainSkills(data.fast_brain || []);
        setFastBrainConfigured(data.fast_brain_configured || false);
      }
    } catch (err) {
  ...
  const handleEdit = async (assistantId: string) => {
    ...
    try {
      ...
      const loadedTtsProvider = (assistant as any).tts_provider || 'cartesia';
      setTtsProvider(loadedTtsProvider);
      // Set voice ID, falling back to provider default if not set
      const loadedVoiceId = assistant.voice_id || (TTS_PROVIDERS[loadedTtsProvider]?.default_voice || 'f786b574-daa5-4673-aa0c-cbe3e8534c02');
  ...
  const resetForm = () => {
    ...
    setVoiceId('f786b574-daa5-4673-aa0c-cbe3e8534c02');
    setSkill('default');
    setLlmProvider('groq');
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 p-4 bg-oled-dark/50 rounded-lg border border-gold/20">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-medium text-gold">Voice Provider & Voice</h4>
                  <span className={`text-xs px-2 py-0.5 rounded ${
                    TTS_PROVIDERS[ttsProvider]?.latency.includes('30') ? 'bg-green-500/20 text-green-400' :
                    TTS_PROVIDERS[ttsProvider]?.latency.includes('60') || TTS_PROVIDERS[ttsProvider]?.latency.includes('80') ? 'bg-yellow-500/20 text-yellow-400' :
                    'bg-orange-500/20 text-orange-400'
                  }`}>
                    {TTS_PROVIDERS[ttsProvider]?.latency || '~100ms TTFB'}
                  </span>
                </div>
                <p className="text-xs text-gray-500">
                  {TTS_PROVIDERS[ttsProvider]?.description || 'Select a voice provider'}
                </p>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (TTS_PROVIDERS[provider]) {
                          setVoiceId(TTS_PROVIDERS[provider].default_voice);
                        }
                      }}
                    ...
                    >
                      <optgroup label="Recommended (Low Latency)">
                        <option value="cartesia">Cartesia (Recommended)</option>
                        <option value="rime">Rime AI (Free Tier)</option>
                  ...
                  <div>
                    <label className="block text-xs text-gray-400 mb-1">
                      Voice {loadingVoices && <span className="text-gold animate-pulse">(loading...)</span>}
                    </label>
                    <select
                      value={voiceId}
                      ...
                      className="w-full px-3 py-2 bg-oled-dark border border-gold/30 rounded-lg
                        ...
                        disabled:opacity-50"
                    >
                      {/* User's Custom Voice Clones - shown first with distinct styling */}
                      {userVoiceClones.length > 0 && (
                        <optgroup label="⭐ Your Voice Clones">
                          {userVoiceClones.map((voice) => (
                      ...
                      {/* Provider Voices grouped by gender */}
                ...
                <div className="flex items-center justify-between text-xs pt-1">
                  <span className="text-gray-500">
                    Selected: <span className="text-gold">
                      {userVoiceClones.find(v => v.id === voiceId)?.name ||
                       providerVoices.find(v => v.id === voiceId)?.name ||
                       voiceId.substring(0, 20)}
                      {userVoiceClones.find(v => v.id === voiceId) && ' ★'}
                    </span>
                  </span>
                  <a
              ...
              <div>
                ...
                >
                  ...
                  <optgroup label="⬡ HIVE Preloaded">
                    {hivePreloadedSkills.map((s) => (
                      <option key={s.id} value={s.id}>{s.name}</option>
                    ))}
                  </optgroup>
                  {/* Fast Brain Skills (if available) */}
                  {fastBrainSkills.length > 0 && (
                    <optgroup label="🧠 Fast Brain Custom">
                      {fastBrainSkills.map((s) => (
                        <option key={s.id} value={s.id}>{s.name}</option>
                      ))}
                    </optgroup>
                  )}
                  {/* Fallback if nothing loaded */}
                  {hivePreloadedSkills.length === 0 && fastBrainSkills.length === 0 && (
                    <option value="default">Default Assistant</option>
                  )}
                </select>
                <p className="text-xs text-gray-500 mt-1">
                  {fastBrainConfigured
                    ? 'HIVE Preloaded + Fast Brain skills available'
                    : 'HIVE Preloaded skills (configure FAST_BRAIN_URL for custom skills)'
                  }
                </p>
              </div>
